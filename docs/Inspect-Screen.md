...
- Public DOM `id` attributes (the DOM renderer uses its own naming conventions)
- User-facing strings
...
- **DOM renderer exposes semantic IDs as data attributes when opted in.** Pass `showSemanticIds: true` to `renderDom()` to emit `data-intent-screen`, `data-intent-ask`, and `data-intent-action` attributes matching the semantic IDs from `inspectScreen()`.
